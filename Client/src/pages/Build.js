...
const Build = () => {
  ...
  const getBuild = (buildid) => {
    fetch(`https://travelers-scroll.herokuapp.com/builds/build/${buildid}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const getUser = () => {
    Axios({
      ...
      url: `https://travelers-scroll.herokuapp.com/builds/build/${buildid}`,
    }).then((res) => {
  ...
  const handleOnCommentSubmit = (e) => {
    if (!comment) {
      ...
      Axios({
        ...
        withCredentials: true,
        url: `https://travelers-scroll.herokuapp.com/builds/build/${buildid}/newComment`,
      }).then((res) => {
        if (res.data.status === "err") {
  ...
  const handleOnLike = () => {
    if (build.likedUsers.includes(user)) {
      Axios({
        ...
        withCredentials: true,
        url: `https://travelers-scroll.herokuapp.com/builds/build/${buildid}/liked`,
      }).then((res) => {
        if (res.data.status === "err") {
          alert(res.data.message);
          if (res.data.message === "Login Required") {
